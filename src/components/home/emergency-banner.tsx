    ...
    <section className="page-shell py-6" aria-labelledby="emergency-title">
      <div className="relative overflow-hidden rounded-[2rem] bg-[#8e2f25] px-6 py-7 text-white shadow-lift sm:px-8 md:grid md:grid-cols-[1fr_auto] md:items-center md:gap-8 lg:px-10">
        ...
        <div className="relative mt-6 flex flex-wrap items-center gap-4 md:mt-0 md:justify-end">
          <div
            className="flex items-center gap-3 rounded-2xl border border-white/14 bg-black/10 px-4 py-3 backdrop-blur-lg"
            aria-label={
              time
                ? `${time.days} days, ${time.hours} hours, and ${time.minutes} minutes remaining`
                : "Loading the emergency response countdown"
            }
          >
            <Clock3 className="size-4 text-[#ffd8bf]" aria-hidden="true" />
            {countdown.map(([value, label], index) => (
              <span key={label} className="flex items-baseline gap-1"><strong className="font-display text-xl">{value === undefined ? "--" : String(value).padStart(2, "0")}</strong><span className="text-[0.58rem] uppercase tracking-wider text-white/45">{label}</span>{index < 2 ? <span className="ml-2 text-white/25">:</span> : null}</span>
            ))}
          </div>
