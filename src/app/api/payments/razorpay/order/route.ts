...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    enforceRateLimit("razorpay-order", getClientIp(request), {
      limit: 10,
      windowMs: 10 * 60_000,
    });
    ...
    const result = await startDonationCheckout(donationId, "razorpay");
    return jsonSuccess(result, {
      status: 201,
      meta: { paymentInitiated: true },
    });
  } catch (error) {
    return errorResponse(error, requestId, "razorpay-order");
  }
}
